...
import (
	...
	"github.com/hello/planet/testutil/network"
	"github.com/hello/planet/x/blog/client/cli"
...
func networkWithSentPostObjects(t *testing.T, n int) (*network.Network, []*types.SentPost) {
	...
	for i := 0; i < n; i++ {
		state.SentPostList = append(state.SentPostList, &types.SentPost{Creator: "ANY", Id: uint64(i)})
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.Equal(t, tc.obj, resp.SentPost)
			}
		})
	}
}
...
func TestListSentPost(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			for j := i; j < len(objs) && j < i+step; j++ {
				assert.Equal(t, objs[j], resp.SentPost[j-i])
			}
		}
	})
	...
	t.Run("Total", func(t *testing.T) {
		...
		require.Equal(t, objs, resp.SentPost)
	})
}
